...
,
		...
		methods:{
			...
			getStaffLocation(){
				...
				uni.getLocation({
					...
					success: function (res) {
						...
						uni.$emit('getLoaction',{loaction:location})
				...
				uni.request({
						...
						success:res=>{
							...
							if(res.data.code == 1){
								...
								for(let i in _this.shopMark){
									for(let j in _this.shopMark[i]){
										markId++
										this.$set(_this.shopMark[i][j],'markId',markId)
									}
								}
			...
			handleMarker(e){
				let idx = Number(e.detail.markId)
				console.log(e,JSON.stringify(this.markArr))
			},
			// 循环添加marker
			addMarker(type){
				
				let iconUrl = '';
				let markdata = new Object();
				this.markTempArr = [];
				this.markArr = []
				...
				for(let i in markdata){
					...
					switch(i){
						case 'dutyShop': iconUrl = '/static/images/1.png';
						break;
					...
					// 	})
					// 	this.markTempArr.map((item,idx)=>{
					// 		this.$set(item,'id',idx)
					// 	})
					// }
