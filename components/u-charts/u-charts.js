...
function drawToolTipSplitLine(offsetX, opts, config, context) {
  ...
  context.setStrokeStyle(toolTipOption.gridColor || '#000');
  context.setLineWidth(1 * opts.pixelRatio);
  context.beginPath();
      ...
      case 'group':
        ...
        points = fixColumeData(points, eachSpacing, series.length, seriesIndex, config, opts);
          ...
          if (item !== null && i>leftNum && i<rightNum) {
			...
			grd.addColorStop(0, "#FC755B");
			grd.addColorStop(1, "#EE453C"); 
			if(data[i] != ''){
				context.setStrokeStyle(item.color || grd);
				context.setFillStyle(item.color || grd);
			}else{
...
function drawYAxis(series, opts, config, context) {
  ...
  for (let i = 0; i < opts.yAxis.data.length; i++) {
    ...
    if(yData.disabled !== true){
      ...
      //画Y轴轴线
      if (opts.yAxis.axisLine !== false) {
        context.beginPath();
        context.setStrokeStyle(opts.yAxis.axisLineColor||'#cccccc');
        context.setLineWidth(1 * opts.pixelRatio);
        if(yAxisWidth.position=='left'){
	  ...
	  // if(yData.axisLine !== false ){
		 //  context.beginPath();
		 //  context.setStrokeStyle(opts.yAxis.axisLineColor);
		 //  context.setLineWidth(1 * opts.pixelRatio);
		 //  if(yAxisWidth.position=='left'){
		 //    context.moveTo(tStartLeft,opts.height-opts.area[2]);
		 //    context.lineTo(tStartLeft,opts.area[0]);
